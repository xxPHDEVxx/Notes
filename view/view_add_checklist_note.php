        ...
        <div class="container add_checklist">
            ...
            <div class="mb-3">
                ...
                <ul>
                    <?php for ($i = 0; $i < 5; $i++) : ?>
                        <li>
                        <input type="text" class="form-control item_add <?= (!empty($errors["item_$i"]) ? "border border-danger" : (isset($_POST['items'][$i]) && !empty($_POST['items'][$i]) ? "border border-success" : "")) ?>" name="items[]" value="<?= isset($_POST['items'][$i]) ? htmlspecialchars($_POST['items'][$i]) : "" ?>">                            <?php if (!empty($errors["item_$i"])) : ?>
